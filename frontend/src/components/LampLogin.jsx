...
export default function LampLogin({ onLoginSuccess }) {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#0b0806] isolate flex items-center justify-center">
      ...
      {/* =========================================================
          LAYER 0: BACKDROP PENUH LAYAR (mengisi area letterbox
          di luar scene container agar tidak ada gap warna solid polos)
          ========================================================= */}
      ...
      {/* =========================================================
          SCENE CONTAINER — PROPORSI TETAP (aspect-ratio 16:9)
          Semua elemen scene (lampu, branding, panel login) berada
          DI DALAM container ini dan diposisikan relatif terhadapnya
          (persen terhadap container, bukan terhadap layar penuh).
          Ini yang membuat seluruh scene ter-scale BERSAMAAN pada
          rasio apa pun — HP potret, tablet, laptop lebar, dsb —
          sehingga posisi relatif lampu vs teks vs panel login
          selalu konsisten dan sakelar selalu berada di lokasi yang
      ...
      >
        {/* =========================================================
            ✨ BRANDING MAMET OS — TENGAH ATAS SCENE
            ========================================================= */}
        <div
          className="absolute top-[16%] left-1/2 -translate-x-1/2 flex items-center gap-[1.2%] pointer-events-none z-10 select-none whitespace-nowrap"
        >
          <Cpu
            className="text-[#ffe3b3] drop-shadow-[0_0_15px_rgba(255,220,150,0.4)]"
            style={{ width: 'clamp(24px, 4vw, 64px)', height: 'clamp(24px, 4vw, 64px)' }}
          />
          <span
            className="text-[#fff8ec] tracking-[0.12em] uppercase font-mono font-light"
            style={{
              fontSize: 'clamp(14px, 2.6vw, 48px)',
              textShadow: `
        ...
        <div className="absolute inset-x-0 bottom-[13%] h-[0.3%] z-[6] bg-[#0d0805] shadow-[0_-1px_3px_rgba(255,178,86,.09),0_3px_14px_#000]" />

        {/* =========================================================
            LAMP SVG (Fisik Lampu & Sakelar)
            Diposisikan & diukur 100% relatif terhadap scene container,
            sehingga tidak akan pernah terpotong oleh viewport dan
            area klik sakelar selalu presisi di semua ukuran layar.
            ========================================================= */}
        <div className="absolute left-[4%] bottom-[12.5%] w-[46%] h-[68%] z-[10] drop-shadow-[10px_14px_12px_rgba(0,0,0,.55)]">
          <svg viewBox="0 0 560 680" className="w-full h-full overflow-visible" preserveAspectRatio="xMidYMax meet">
            <defs>
            ...
            <g
              ...
              filter="url(#switchShadow)"
              onClick={toggleLamp}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleLamp(); } }}
              className="cursor-pointer hover:drop-shadow-[0_0_5px_rgba(255,191,91,.25)] active:translate-y-[3px] transition-transform duration-100"
            >
              {/* Hit-area lebih besar dari tampilan visual sakelar,
                  supaya tetap mudah disentuh di layar kecil/HP */}
              <circle cx="170" cy="595" r="26" fill="transparent" />
        ...
        {/* =========================================================
            ...
            ========================================================= */}
        <div className={`absolute z-[15] top-[8%] right-[5%] w-[38%] min-w-[260px] max-w-[430px] p-[5%] rounded-[1.5vw] transition-all duration-600 ${lampOn ? 'opacity-100 scale-100 visible pointer-events-auto' : 'opacity-0 scale-95 invisible pointer-events-none'}`}>
