...
export default function AIAgent() {
  ...
  // --- CONVERSATION INSPECTOR HELPERS ---
  const getLatestAgentMessage = (msgs) => {
    if (!msgs || !msgs.length) return null;
    for (let i = msgs.length - 1; i >= 0; i--) {
      if (msgs[i].type === 'agent') return msgs[i];
    }
    return null;
  };

  const getInspectorData = (message) => {
    if (!message) return null;
    return {
      execution: message.processingSteps || [],
