...
const GenericAppWrapper = ({ appId, workspaceId, mainPanel: MainPanel }) => (
  <WorkspaceProvider appId={appId} defaultWorkspaceId={workspaceId}>
    <AppShell mainPanel={<Suspense fallback={<div className="p-4 text-emerald-500">Loading Module...</div>}><MainPanel /></Suspense>} />
  </WorkspaceProvider>
);

const lazyWithWrapper = (importFunc) => {
  const LazyComponent = lazy(importFunc);
  return (props) => <GenericAppWrapper {...props} mainPanel={LazyComponent} />;
};
