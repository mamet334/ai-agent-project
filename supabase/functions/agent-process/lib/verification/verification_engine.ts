...
export class VerificationEngine {
  ...
  /**
   * Mode-Aware Verification for OWNER/LITE modes.
   * Based on Capability Separation in Mamet AI Constitution (MAEF 4.5).
   * Relaxes strict engineering rules (like Source Trace and ADR format) 
   * to allow normal conversation and dynamic memory retrieval.
   */
  static verifyPersonal(context: VerificationContext): VerificationReport {
    ...
    console.log(`[VERIFICATION-PERSONAL]\n${check001.id}\n${check001.status}`);

    // ---------------------------------------------------------
    // SOURCE_TRACE checks removed for Mode-Aware Verification
    // ---------------------------------------------------------
